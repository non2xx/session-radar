...
- **Live status** per session, read from the tmux pane title Claude itself sets:
  ...
  - **dashed ring (blue)**, a spinning gear in the tree view — *agents*: **you can talk.** The main conversation is free; only subagents it dispatched are still running. The card shows how many, and the tooltip names them. The pane title cannot tell this apart — Claude spins its title spinner for a subagent too — so this one state is read from the pane's screen instead (see below).
...
- **Subagent tree** — expand a session to see **what it has running behind it**: one row per subagent with the job it was given and how long since it last moved. The list comes from `claude agents --json`, matched to a tmux session **by name** — or by folder, if you gave the session a project path. So an empty list usually means one of two things: the names don't line up (fix it with "프로젝트 경로 지정"), or VS Code can't see your `claude` on `PATH` (nvm installs are the usual cause — put the full path from `which claude` in `sessionRadar.claudeCommand`). The name gets the whole row — hover for the full text plus its type (`general-purpose`, `my-plugin:reviewer`, …). A session with a live subagent shows `●3` and starts expanded; one with only past runs starts collapsed; one that never dispatched any has no arrow at all. Agents that dispatched their own agents nest a level deeper. Long job names are cut to ~20 characters — hover for the full text. Toggle with `sessionRadar.showSubagents`.
- **Stuck sessions** — a **막힌 세션** section lists Claude sessions sitting in `blocked`, with the time since each one **started** (weeks, in practice — it is not the time since it got stuck). In practice these have no tmux window, so they were invisible everywhere else; this is the one place they surface.
...
- **Manage from either view** — create / rename / delete groups, rename (display alias) / hide / add sessions, clear a session's project path, take a session out of its group (card view), and **drag-and-drop** to reorder or regroup. The view title bar carries **"그룹 추가"**, **"세션 추가"**, **"새로고침"** (poll now instead of waiting for the next round) and **"이미지 나란히 보기"**. **"이미지 칸 배치 처음부터"** has no button — call it from the Command Palette when the image grid gets out of step.
...
Every few seconds the extension runs `tmux list-panes` across all sessions and reads each pane's **foreground command and title**. Claude Code puts an animated spinner in its title while it is working and a `✳` when it is your turn, so the title alone tells the state: no hook, no daemon, nothing writing files. A session with several panes takes its strongest state (`working` > `turn` > `inactive`); `agents` is added a step later, per session, by the screen read below.

Which glyphs that spinner uses is a Claude Code build detail that has already changed under us at least once (braille `⠋⠙⠹` on older builds, half-circles `◐◓◑◒` on 2.1.231 — those are examples, not the list). `isSpinnerGlyph` in `src/core/tmux.ts` is the single place that lists them, and it accepts every set seen so far. **If every session suddenly reads as *turn*, that is the symptom of a new set**: check the first character of `tmux list-panes -a -F '#{pane_title}'` and teach it to that one function.

The title alone cannot say **who** is working: Claude spins the same title spinner while a subagent runs, so a session waiting for your next message reads as busy. Only sessions the title already calls *working* then get one `tmux capture-pane`, and the bottom of that screen settles it — a thinking line above the input box means the main conversation is answering, while an agent tray (`● main` plus one row per subagent) with no thinking line means it is free. Everything else is answered by the title, so no extra process is spawned for it; the reads are capped at 250ms per round and cached for a second. If a screen can't be read in time the session simply stays *working*, which is what it reported before.

The subagent tree comes from two other places. `claude agents --json` lists every Claude session with its `sessionId`, `cwd` and state (`status` for interactive ones, `state` for background ones); it costs about half a second, so it is the one thing kept **off** the 3-second loop — its own slower timer (`sessionRadar.agentsRefreshSeconds`, default 20) refreshes it and the views only ever read its cached answer. Each session's subagents are then read from `~/.claude/projects/<cwd with every non-alphanumeric character turned into ->/<sessionId>/subagents/`: the directory listing and each `agent-*.jsonl`'s **modification time** say what is running (measured: 3~5ms for 245 files across 10 sessions), and only the rows about to be drawn have their tiny `agent-*.meta.json` opened for the job name and type. That scan *is* inside the 3-second refresh; what keeps it cheap is a 1-second cache and a 150ms budget per round, so leaving those in place matters. The `.jsonl` transcripts themselves are **never opened** — the largest here is 168MB, and one record can be the whole file.
...
- Drag/grouping live in the panel; tmux itself is only ever read (`list-sessions`, `list-panes`, `capture-pane` — all read-only) or attached-to (`new-session -A`).
- If you move Claude's config folder with `CLAUDE_CONFIG_DIR`, the subagent tree follows it, but session-radar's own `layout.json` / `open.json` still live under `~/.claude/`.
