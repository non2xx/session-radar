...
- **Image links and wrapped lines** — a narrow terminal folds a long path across two visual rows, and the extension only ever sees one row at a time, so neither half is a path that resolves. **Click the first (upper) row**: the extension walks the deepest directory that actually exists and takes the single entry starting with what's left, so a path cut anywhere still opens. If more than one entry matches it does nothing, on the grounds that opening the wrong image is worse than opening none. The lower row stays unclickable — on its own it says nothing about which folder it came from.
